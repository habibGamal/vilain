...
export default function Home({ auth, featuredProducts, announcements, heroSlides, categories, brands }: HomePageProps) {
  const { t } = useLanguage();

  // Filter active categories and brands
  const activeCategories = categories?.filter(cat => cat.is_active) || [];
  const activeBrands = brands?.filter(brand => brand.is_active) || [];
  const bestOffers = featuredProducts?.filter(product => product.sale_price && product.is_active) || [];
  ...
  return (
    <>
      ...
      {/* Category Grid */}
      <CategoryGrid
        categories={activeCategories.slice(0, 10)}
